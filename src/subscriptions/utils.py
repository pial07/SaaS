...
def refresh_active_users_subscription(user_ids=None, 
     ...
     if day_start > -1 and day_end > -1:
          qs=qs.by_range(day_start=day_start,day_end=day_end, verbose=verbose)             
     complete_count=0
     qs_count=qs.count()             
     for obj in qs:
        if verbose:
        ...
        for customer_obj in qs:
            ...
            for sub in subs:
                ...
                if existing_user_sub_qs.exists():
                    continue
                helpers.billing.cancel_subscription(sub.id,reason="Dangling user Subscription",                    cancel_at_period_end=False)
